...
export const useStudentFeesState = () => {
  ...
  const isEdit = !!id;
  const [studFeesDetails, setStudFeesDetails] =
    useState<IStudentFeesDetailsById | null>(null);
        ...
        (err) => {
          toast.error(
            err?.response?.data?.meesage ||
              "Error while updating student Fees details",
          );
        },
      );
    } else {
  ...
  const handleDeleteStudFeeRecord = () => {
    ...
    StudentFeesApiProvider.apolloInstance.deleteStudFees(
      ...
      (res) => {
        ...
        setStuFeesData((prev) =>
          prev.filter((it) => it.id !== selectStudFeeId),
        );
        closeDeleteModal();
      },
      (err) => {
        toast.error(
          err?.response?.data?.meesage || "Error while Deleting Student Record",
        );
      },
    );
  };
