...
export const createStudentFees = async (req: Request, res: Response) => {
  try {
    ...
    const due_amount= total_fee - Number(amount_paid);
...
export const deleteStudentFees = async (
  req:Request,
  res:Response,
)=> {
  try {
...
export const updateStudentFees = async (
  ...
  try {
    ...
    const total_fee = Number(
      feeResult.rows[0].total_fee
    );
    const due_amount =
      total_fee - Number(amount_paid);
    let payment_status = "Pending";
    ...
    res.status(500).json({
      success: false,
      message:
        "Error while updating student fees",
    });
  }
};