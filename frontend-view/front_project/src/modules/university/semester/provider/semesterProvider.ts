...
export class SemesterApiProvider extends AxiosService {
  ...
  async createSemester(
    ...
    try {
      const res = await this.post<ISemesterResponse>(
  ...
  async getSemesterById(
    ...
    try {
      const res = await this.get(`${SEMESTER.GETSEMESTER}/${id}`);
  ...
  async updateSemester(
    ...
    try {
      const res = await this.put(`${SEMESTER.UPDATESEMESTER}/${id}`, params);
  ...
  async deleteSemester(
    ...
    try {
      const res = await this.delete(`${SEMESTER.DELETESEMESTER}/${id}`);
